...
async function loadChain() {
  ...
  clearData(); loading = true; const token = epoch; $('#network-details').open=false; $('#workspace-status').hidden=false; $('#workspace-status').textContent='Loading your draws...'; $('#chain-status').textContent='Reading connected network and contract...'; render();
  try {
    ...
    $('#chain-status').innerHTML = `Connected to ${escape(config.chainName)}. Contract: <a class="contract-link" href="${explorer('address',config.contractAddress)}" target="_blank" rel="noopener noreferrer">${escape(shortAddress(config.contractAddress))} ↗</a>`;
    $('#workspace-status').hidden=true; $('#draw-source').textContent = `On-chain snapshot at block ${block.toLocaleString()}`;
    const shared = new URLSearchParams(location.hash.split('?')[1] || '').get('draw'); if(shared && draws.some(d=>d.id===shared)) openDraw(shared);
  } catch(error) { if(token===epoch){ $('#workspace-status').hidden=false; $('#workspace-status').textContent='Unable to load draws. Check Network details or try refreshing.'; $('#chain-status').textContent = errorMessage(error); $('#draw-source').textContent = 'On-chain data unavailable'; } }
...
function openDraw(id) {
  ...
  modal(d.title,`<div class="detail-pills"><span>Draw #${d.id}</span><span>${d.participants.length} entries</span><span>Free participation</span></div><p class="modal-copy">Creator: <span class="participant-address">${escape(d.creator)}</span><br>Registration deadline: ${new Date(d.deadline).toLocaleString()}</p>${d.state===2?`<div class="result">RECORDED WINNER<strong class="address-result">${escape(d.winner)}</strong></div><button class="primary" id="verify-proof">◇ Verify result</button> <button class="text-button" id="share-result">Share result ↗</button><div id="verification"></div>`:d.state===1?'<p class="notice">Winner selection is in progress. Refresh to check for a winner.</p>':Date.now()<d.deadline?`${!randomnessReady?'<p class="notice">Winner selection isn&#8217;t available yet.</p>':''}<p class="modal-copy">Register ${escape(shortAddress(account))}. One entry per wallet. Network gas fees apply.</p><button class="primary" id="join-draw" ${joined?'disabled':''}>${joined?'Already registered':'Register wallet ↗'}</button>`:`<p class="notice">Registration is closed. ${randomnessReady?'You can now select a winner.':'Winner selection isn&#8217;t available yet.'}</p><div class="modal-actions"><button class="primary" id="select-winner" ${!d.participants.length||!randomnessReady?'disabled':''}>Select winner &#8599;</button></div>${!d.participants.length?'<p class="modal-copy">No participants registered. A winner cannot be selected.</p>':''}`}<div class="section-heading" style="margin-top:25px"><h3>On-chain participant list</h3><button class="text-button" id="export-list">CSV ↓</button></div><ul class="participant-list">${d.participants.map((p,i)=>`<li><span class="participant-address">${escape(p)}</span><small>#${i+1}</small></li>`).join('')||'<li>No registered wallets.</li>'}</ul><a class="tx-link" href="${explorer('address',config.contractAddress)}" target="_blank" rel="noopener noreferrer">View contract on explorer ↗</a>`);
